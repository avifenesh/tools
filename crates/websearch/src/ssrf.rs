...
pub async fn classify_host(host: &str, session: &WebSearchSessionConfig) -> SsrfDecision {
    let addresses = match resolve_host(host).await {
        Ok(addrs) => addrs,
        Err(e) => {
            return SsrfDecision::Blocked {
                reason: format!("DNS resolution failed: {}", e),
                hint: "Check that the backend hostname is reachable and correct.".to_string(),
            };
        }
    };
    if addresses.is_empty() {
        return SsrfDecision::Blocked {
            reason: "Backend hostname did not resolve to any address.".to_string(),
            hint: "Check DNS or session.searxng_url.".to_string(),
        };
    }
    for addr in &addresses {
        if let Some(class) = classify_ip(*addr) {
            if !opted_in(class, session) {
                return SsrfDecision::Blocked {
                    reason: format!(
                        "Backend resolved to blocked IP range: {} ({})",
                        addr,
                        class.as_str()
                    ),
...
fn opted_in(class: BlockClass, session: &WebSearchSessionConfig) -> bool {
    match class {
...
fn hint_for(class: BlockClass) -> &'static str {
    match class {
        BlockClass::Loopback => {
            "Loopback is blocked by default. A self-hosted SearXNG usually runs on localhost — the session must set allow_loopback: true to permit it."
        }
        BlockClass::Private => {
            "Private IP ranges (RFC 1918) are blocked by default. For a SearXNG on the LAN, set session.allow_private_networks: true."
        }
        ...
        BlockClass::Metadata => {
            "Cloud metadata endpoints (169.254.169.254) are blocked by default to prevent credential exfiltration. A metadata endpoint is not a search engine; set session.allow_metadata: true only if you really mean it."
        }
        BlockClass::Reserved => {
            "Reserved / special-purpose IP range (0.0.0.0/8, broadcast, etc.) — not a useful backend target."
        }
    }
}
