...
import type { GlobSessionConfig } from "./types.js";
...
export async function fenceGlob(
  ...
  if (isSensitive && permissions.hook === undefined) {
    return toolError(
      "SENSITIVE",
      `Refusing to glob sensitive path: ${resolvedPath}`,
  ...
  if (permissions.hook !== undefined) {
    ...
    if (decision === "deny") {
      return toolError(
        "PERMISSION_DENIED",
        `Glob denied by user: ${resolvedPath}`,
        { meta: { path: resolvedPath } },
      );
    }
  }
...
export function resolveOps(session: GlobSessionConfig): ReadOperations {
